...
| setState | `values` | A space-separated list.

$0 <=$ `values[i]` $<= 8$ for $i = 1, ..., 8$. 

`len(values)` $= 9$.

`values[i] != values[j]` for $0 <= i, j <= 8$ | Set the state of the eight-puzzle. |
| scrambleState | `n` | An integer. `n >= 0`. | Perform `n` random moves on the eight-puzzle. |
