...
| setState | `values` | A space-separated list. <br> $0 <=$ `values[i]` $<= 8$. <br> `len(values)` $= 9$. <br> `values[i] != values[j]` | Set the state of the eight-puzzle. | scrambleState | `n` | An integer. `n >= 0`. | Perform `n` random moves on the eight-puzzle. |
